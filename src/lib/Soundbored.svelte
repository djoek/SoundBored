<script lang="ts">
  ...
  function initializePads() {
    ...
    // For 12x2 grid, we want to map to 2 octaves (24 notes)
    if (selectedGridSize === GRID_SIZES.GRID_12X2) {
      // Create 24 pads (12x2 grid) - 2 octaves
  ...
  function getMidnightStyles(pad) {
  ...
  {#if showSoundAssignModal}
    <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div class="bg-gray-800 p-6 rounded-lg max-w-md w-full">
        ...
        <div class="mb-4">
          <label class="block text-white mb-2">Select WAV or audio file:</label>
          <input
            type="file"
            accept="audio/*"
